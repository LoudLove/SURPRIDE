...
def split_data(data, val_size=0.15, test_size=0.15):
    """
    按照 0.7 : 0.15 : 0.15 的比例切分数据集为 train, val, test。
    """
    # 首先，将数据集划分为训练集和临时集（剩下的部分）
...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 获取图像信息
        image_name = self.metadata.iloc[idx]['dis_img_path']  # 使用图像名称
        mos_score = self.metadata.iloc[idx]['score']

        # 从路径中提取文件名（去掉路径部分）
        image_name = os.path.basename(image_name)  # 提取文件名
        ...
        original_img_path = os.path.join(self.original_path, image_name)

        # 加载图像
        original_img = Image.open(original_img_path).convert("RGB")   #496*496
...
def search_hyperparameters():
    ...
    # 读取 .txt 文件数据，假设文件是用逗号分隔的 CSV 格式
    txt_file_path = r'J:\DATASET\datasets\LIVEChallenge_del_t7\LIVE_Challenge.txt'  # 替换为实际的文件路径
    data = pd.read_csv(txt_file_path)

    # 使用 split_data 函数切分数据
    train_data, val_data, test_data = split_data(data, val_size=0.15, test_size=0.15)
    ...
    # 原图路径和超分路径
    origin_img_path = r'J:\DATASET\datasets\LIVEChallenge_del_t7\Images'
    SR_paths = {
            'SRx4': r'J:\DATASET\datasets\LIVEChallenge_SwinFIRx4\Images'
    }

    # 更新metadata
    train_metadata = train_data[['dis_img_path', 'score']].reset_index(drop=True)
    val_metadata = val_data[['dis_img_path', 'score']].reset_index(drop=True)
    test_metadata = test_data[['dis_img_path', 'score']].reset_index(drop=True)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'LIVEC_wo_SR_224.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('LIVEC_wo_SR_224.pth'))
