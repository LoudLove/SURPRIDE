...
from transformers import ViTImageProcessor, ResNetForImageClassification, AutoModelForImageClassification,DeiTForImageClassificationWithTeacher
import torch
...
class MultiViTRegressor(nn.Module):
    def __init__(self, model_name="", feat_weight=0.05):
        super(MultiViTRegressor, self).__init__()
        self.feat_weight = feat_weight

        # 初始化6个独立的 ViT 模型
        self.model1 = DeiTForImageClassificationWithTeacher.from_pretrained(model_name)
        self.model2 = DeiTForImageClassificationWithTeacher.from_pretrained(model_name)
...
def search_hyperparameters():
    ...
    # 初始化模型，传递feat_weight
    model = MultiViTRegressor(model_name='facebook/deit-base-distilled-patch16-224', feat_weight=0.1).to(
        device)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'UHDIQA_use_deit224.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('UHDIQA_use_deit224.pth'))
