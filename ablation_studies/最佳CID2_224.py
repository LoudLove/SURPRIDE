...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        SRx4_img_path = os.path.join(self.SR_paths['SRx4'], folder_name, subfolder_name, image_name_png)
        ...
        SRx4_img = Image.open(SRx4_img_path).convert("RGB") #1600*1200
...
def search_hyperparameters():
    ...
    for epoch in range(15):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch + 1} - Training", unit="batch") as pbar:
            for a, b, c, d, mos_score in pbar:
                ...
                # 总损失 = MSE损失 + 余弦相似度损失
                total_loss_per_batch1 = mse_loss + lambda_1 * (1 - torch.abs(cos_simi))  # 用 lambda_1 作为余弦相似度的权重
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'CID_224.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('CID_224.pth'))
