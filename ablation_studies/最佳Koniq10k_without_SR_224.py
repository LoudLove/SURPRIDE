...
def read_mos_data(mos_csv_path):
    """读取 MOS 数据，并按比例划分数据集"""
    # 读取 MOS 数据
    mos_data = pd.read_csv(mos_csv_path)
...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mos_score = self.metadata.iloc[idx]['MOS']
        ...
        # 加载图像
        original_img = Image.open(original_img_path).convert("RGB") #1024*768
...
def search_hyperparameters():
    ...
    # 超分图像路径
    SR_paths = {
        'SRx4': r'J:\DATASET\datasets\Koniq10k_SwinFIRx4\1024x768'
    }

    train_data, val_data, test_data = read_mos_data(r'J:\DATASET\datasets\koniq10k\koniq10k_scores_and_distributions.csv')

    train_metadata = train_data[['image_name', 'MOS']].reset_index(drop=True)
    val_metadata = val_data[['image_name', 'MOS']].reset_index(drop=True)
    test_metadata = test_data[['image_name', 'MOS']].reset_index(drop=True)
    original_path=r'J:\DATASET\datasets\koniq10k\1024x768'

    # 调用 IQAdataset 类
    train_dataset = IQAdataset(
        metadata=train_metadata,
        original_path=original_path,  # 原图路径
        SR_paths=SR_paths
    )
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'Koniq10k_wo_SR_224.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('Koniq10k_wo_SR_224.pth'))
