...
class MultiViTRegressor(nn.Module):
    def __init__(self, model_name="facebook/convnext-base-384-22k-1k", feat_weight=0.05):
        ...
        # 添加一个全连接层，将 768 维的 feat2 映射到 1024 维
        self.fc = nn.Linear(768, 1024)

        # 定义回归头：输入为拼接的特征
        self.regressor = nn.Sequential(
            nn.Linear(1024 + 1024, 512),  # 输入特征大小为 1024 + 768
            nn.Linear(512, 1)  # 回归输出为 1
        )

    def forward(self, img1, img2):
        # 通过每个 ViT 模型提取特征
        feat1 = self.model1(img1).logits  # origin 从 DeiT 提取特征
        feat2 = self.model2(img2).logits  # SR 从 ConvNext 提取特征

        # 使用全连接层将 feat2 从 768 维映射到 1024 维
        feat1 = self.fc(feat1)
...
def search_hyperparameters():
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'UHDIQA_origin_deit_sr_conv.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('UHDIQA_origin_deit_sr_conv.pth'))
