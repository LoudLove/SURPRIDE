...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        SRx4_img_path = os.path.join(self.SR_paths['SRx4'], image_name_png)
        ...
        SRx4_img = Image.open(SRx4_img_path).convert("RGB")
...
def search_hyperparameters():
    ...
    # 初始化模型，传递feat_weight
    model = MultiViTRegressor(model_name="facebook/convnext-base-224-22k-1k", feat_weight=0.1).to(
        device)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'LIVEC_wo_disloss_224.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('LIVEC_wo_disloss_224.pth'))
