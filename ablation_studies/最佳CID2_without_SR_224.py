...
def read_mos_data(mos_xls_path):
    """读取 MOS 数据，并按比例划分数据集"""
    # 使用 pandas 的 read_excel 方法来读取 .xls 文件
    mos_data = pd.read_excel(mos_xls_path, sheet_name='CID2013 MOS')  # 读取 cid2013 mos 表格

    # 按照 0.7:0.15:0.15 划分数据集（训练集、验证集、测试集）
    train_data, temp_data = train_test_split(mos_data, test_size=0.3, random_state=42)
...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        # 获取图像信息
        source_id = self.metadata.iloc[idx]['Source_ID']
        mos_score = self.metadata.iloc[idx]['Realigned MOS']

        # 格式化图像路径（根据 Source_ID）
        image_name = f"{source_id}.jpg"  # 将 Source_ID 加上 .jpg 后缀
        ...
        # 构建图像路径
        # image_name 直接对应 CID2013 数据集中的路径
        # 假设 CID2013 的目录结构：root_dir/IS6/co8/co0\\IS_VI_C08_D12.jpg
        parts = source_id.split('_')  # 将 Source_ID 拆分
        # 获取文件夹名称

        arabic_number = self.roman_to_arabic.get(parts[1], 0)  # 转换为 6，如果不存在则返回 0
        folder_name = f"IS{arabic_number}"  # 构建 IS6
        subfolder_name = f"co{int(parts[2][1:])}"  # 提取 C08 中的数字并构建 co8
        original_img_path = os.path.join(self.original_path, folder_name, subfolder_name, image_name)

        # 加载图像
        original_img = Image.open(original_img_path).convert("RGB")  #1600*1200
        ...
        # 构造返回数据
        data = (
            original_img_splice_tensor,
            original_img_splice_tensor2,
...
class MultiViTRegressor(nn.Module):
    def __init__(self, model_name="facebook/convnext-base-224-22k-1k", feat_weight=0.05):
...
def search_hyperparameters():
    ...
    # 超分图像路径
    SR_paths = {
        'SRx4': r'J:\DATASET\datasets\CID2013_SwinFIRx4'
    }

    # 读取 MOS 数据
    train_data, val_data, test_data = read_mos_data(r'J:\DATASET\datasets\CID2013\CID2013 data - version 12112014.xlsx')
    origin_img_path = r'J:\DATASET\datasets\CID2013'

    # 训练数据
    train_metadata = train_data[['Source_ID', 'Realigned MOS']].reset_index(drop=True)
    val_metadata = val_data[['Source_ID', 'Realigned MOS']].reset_index(drop=True)
    test_metadata = test_data[['Source_ID', 'Realigned MOS']].reset_index(drop=True)

    # 调用 IQAdataset 类
    train_dataset = IQAdataset(
        metadata=train_metadata,
        original_path=origin_img_path,  # 原图路径
        SR_paths=SR_paths
    )
    ...
    # 初始化模型，传递feat_weight
    model = MultiViTRegressor(model_name="facebook/convnext-base-384-22k-1k", feat_weight=0.1).to(
        device)
    ...
    for epoch in range(15):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch + 1} - Training", unit="batch") as pbar:
            for a, b, mos_score in pbar:
                # 将数据迁移到设备
                a, b, mos_score = a.to(device), b.to(device), mos_score.to(device)

                optimizer.zero_grad()

                # 前向传播
                output1 = model(a)
                output2 = model(b)  # for second input pair
                ...
                # 总损失 = MSE损失 + 余弦相似度损失
                total_loss_per_batch1 = mse_loss
                total_loss_per_batch2 = mse_loss2
        ...
        with torch.no_grad():
            with tqdm(val_loader, desc=f"Epoch {epoch + 1} - Validation", unit="batch") as pbar:
                for a, b, mos_score in pbar:
                    ...
                    # 总损失 = MSE损失 + 余弦相似度损失
                    total_loss_per_batch1 = mse_loss1
                    total_loss_per_batch2 = mse_loss2
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'CID_wo_SR_224.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('CID_wo_SR_224.pth'))
