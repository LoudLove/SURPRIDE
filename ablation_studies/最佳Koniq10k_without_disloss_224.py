...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        SRx4_img = Image.open(SRx4_img_path).convert("RGB") #1024*768
...
def search_hyperparameters():
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'Koniq10k_wo_disloss_224.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('Koniq10k_wo_disloss_224.pth'))
