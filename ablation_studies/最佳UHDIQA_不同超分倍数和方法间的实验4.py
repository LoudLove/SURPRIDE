...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        SRx4_img_path = os.path.join(self.SR_paths['HAT_SRx2'], self.data_set, image_name_png)
...
def search_hyperparameters():
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'UHDIQA_exp_SR234_and_diff_SR_method_4.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('UHDIQA_exp_SR234_and_diff_SR_method_4.pth'))
