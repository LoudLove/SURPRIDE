...
def search_hyperparameters():
    ...
    # 初始化模型，传递feat_weight
    model = MultiViTRegressor(model_name="facebook/convnext-base-384-22k-1k", feat_weight=0.5).to(
        device)
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'UHDIQA_exp_lambda_alpha_15.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('UHDIQA_exp_lambda_alpha_15.pth'))
