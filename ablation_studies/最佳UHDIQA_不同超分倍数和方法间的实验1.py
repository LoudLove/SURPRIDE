...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        SRx4_img_path = os.path.join(self.SR_paths['SwinFIR_SRx2'], self.data_set, image_name_png)
...
def search_hyperparameters():
    ...
    # 超分图像路径
    SR_paths = {
        'SwinFIR_SRx2': r'J:\DATASET\SwinFIRx2_uhdiqa',  ### 1
        'SwinFIR_SRx3': r'J:\DATASET\SwinFIRx3_uhdiqa',  ### 2
        'SwinFIR_SRx4': r'J:\DATASET\SwinFIRx4_uhdiqa',  ### 以此类推
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'UHDIQA_exp_SR234_and_diff_SR_method_1.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('UHDIQA_exp_SR234_and_diff_SR_method_1.pth'))
