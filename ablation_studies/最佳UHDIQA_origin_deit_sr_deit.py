...
import time
from transformers import AutoFeatureExtractor, DeiTForImageClassificationWithTeacher, ConvNextForImageClassification
...
class IQAdataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        SRx4_img_path = os.path.join(self.SR_paths['SRx4'], self.data_set, image_name_png)
...
class MultiViTRegressor(nn.Module):
    def __init__(self, model_name="facebook/convnext-base-384-22k-1k", feat_weight=0.05):
        ...
        # 初始化 ConvNext 和 DeiT 模型
        self.model1 = DeiTForImageClassificationWithTeacher.from_pretrained('facebook/deit-base-distilled-patch16-384')
        self.model2 = DeiTForImageClassificationWithTeacher.from_pretrained('facebook/deit-base-distilled-patch16-384')

        # 替换分类头为 Identity
        self.model1.cls_classifier = nn.Identity()
        self.model1.distillation_classifier = nn.Identity()
        self.model2.cls_classifier = nn.Identity()
        self.model2.distillation_classifier = nn.Identity()

        # 定义回归头：输入为拼接的特征
        self.regressor = nn.Sequential(
            nn.Linear(768 + 768, 512),  # 输入特征大小为 1024 + 768
            nn.Linear(512, 1)  # 回归输出为 1
        )

    def forward(self, img1, img2):
        # 通过每个 ViT 模型提取特征
        feat1 = self.model1(img1).logits  # 从 ConvNext 提取特征
        feat2 = self.model2(img2).logits  # 从 DeiT 提取特征
...
def search_hyperparameters():
    ...
    # 超分图像路径
    SR_paths = {
        'SRx2': r'J:\DATASET\SwinFIRx2_uhdiqa',
        'SRx3': r'J:\DATASET\SwinFIRx3_uhdiqa',
        'SRx4': r'J:\DATASET\SwinFIRx4_uhdiqa'
    }
    ...
    if best_model_state is not None:
        torch.save(best_model_state, 'UHDIQA_origin_deit_sr_deit.pth')
        print("Best model saved based on validation performance ")

    # 在测试前，加载保存的最佳模型
    if best_model_state is not None:
        model.load_state_dict(torch.load('UHDIQA_origin_deit_sr_deit.pth'))
